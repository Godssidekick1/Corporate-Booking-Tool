...
export default function TmcCompanyDetailPage() {
  ...
  const [form, setForm] = useState({
    name: '', timezone: '', currency: '', country: '', booking_mode: 'sbt' as Company['booking_mode'],
  })

  useEffect(() => {
    fetch(`/api/tmc/companies/${companyId}`)
      .then(r => r.json())
      .then(data => {
        if (!data.ok) {
...
const s: Record<string, React.CSSProperties> = {
  root: { maxWidth: '600px', fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif" },
